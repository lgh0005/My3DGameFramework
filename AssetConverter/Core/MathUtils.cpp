...
//   AABB 좌표를 얻어오는 유틸 함수   //
//==================================*/
	vec3 Math::Min(const vec3& a, const vec3& b)
	{
		// 각 성분별로 최솟값을 취하여 새로운 vec3를 생성
		return vec3
		(
			glm::min<float>(a.x, b.x),
			glm::min<float>(a.y, b.y),
			glm::min<float>(a.z, b.z)
		);
	}

	vec3 Math::Max(const vec3& a, const vec3& b)
	{
		// 각 성분별로 최댓값을 취하여 새로운 vec3를 생성
		return vec3
		(
			glm::max<float>(a.x, b.x),
			glm::max<float>(a.y, b.y),
			glm::max<float>(a.z, b.z)
		);
	}
	...
	//   assimp to glm 유틸 함수   //
	//============================*/
	mat4 Math::ConvertToGLMMat4(const aiMatrix4x4& from)
	{
		mat4 to;
	...
	vec3 Math::ConvertToGLMVec(const aiVector3D& vec)
	{
		return vec3(vec.x, vec.y, vec.z);
	}

	quat Math::ConvertToGLMQuat(const aiQuaternion& pOrientation)
	{
	...
	//================================================*/
	uint16 Math::FloatToHalf(float f)
	{
		uint32 i = *(uint32*)&f;
