...
namespace MGF3D
{
	...
	)
	{
		...
		if (success) MGF_LOG_INFO("Successfully converted -> {}", outputPath);
		return success;
	}
	...
	)
	{
		...
		{
			success = SaveKTX2ToFile(texture, outputPath);
			if (success) MGF_LOG_INFO("Successfully converted from memory -> {}", outputPath);
		}
	...
	//          LDR Helpers            //
	//=================================*/
	uint8* KTXTextureConverter::LoadLDRImage(const String& path, int32& w, int32& h, int32& ch, bool flipY)
	{
		uint8* pixels = stbi_load(path.c_str(), &w, &h, &ch, STBI_rgb_alpha);
		if (!pixels)
		{
			MGF_LOG_ERROR("LDR Load Failed: {}", path);
			return nullptr;
		}
	...
	bool KTXTextureConverter::FillMipmapsLDR(ktxTexture2* texture, uint8* basePixels, int32 w, int32 h, uint32 numLevels)
	{
		...
		for (uint32 level = 0; level < numLevels; ++level)
		{
			...
			if (res != KTX_SUCCESS)
			{
				MGF_LOG_ERROR("KTXConverter: SetImageFromMemory failed at LDR level {} (Error: {})", level, (int32)res);
	...
	//          HDR Helpers            //
	//=================================*/
	float* KTXTextureConverter::LoadHDRImage(const String& path, int32& w, int32& h, int32& ch, bool flipY)
	{
		float* pixels = stbi_loadf(path.c_str(), &w, &h, &ch, STBI_rgb_alpha);
		if (!pixels)
		{
			MGF_LOG_ERROR("HDR Load Failed: {}", path);
			return nullptr;
		}
		...
		for (uint32 level = 0; level < numLevels; ++level)
		{
			...
			if (targetFormat == 97)
			{
				...
				for (usize i = 0; i < numElements; ++i)
					halfBuffer[i] = Math::FloatToHalf(currentPixels[i]);
			...
			if (res != KTX_SUCCESS)
			{
				MGF_LOG_ERROR("KTXConverter: SetImageFromMemory failed at HDR level {} (Error: {})", level, (int32)res);
				if (currentPixels != basePixels) ::free(currentPixels);
				return false;
			}
	...
	ktxTexture2* KTXTextureConverter::CreateKTX2Object(int32 w, int32 h, uint32 numLevels, uint32 vkFormat)
	{
		...
		{
			MGF_LOG_ERROR("KTX2 Creation Failed (Format: {})", vkFormat);
			return nullptr;
		}
		return texture;
	}

	bool KTXTextureConverter::CompressToBasis(ktxTexture2* texture, const String& formatStr, bool isSRGB)
	{
		MGF_LOG_INFO("Compressing LDR to {} ({})...", formatStr, isSRGB ? "sRGB" : "Linear");
	...
	bool KTXTextureConverter::SaveKTX2ToFile(ktxTexture2* texture, const String& path)
	{
		return ktxTexture_WriteToNamedFile(ktxTexture(texture), path.c_str()) == KTX_SUCCESS;
	}

	uint32 KTXTextureConverter::MapFormatStringToVk(const String& formatStr, bool isHDR, const String& colorSpace)
	{
