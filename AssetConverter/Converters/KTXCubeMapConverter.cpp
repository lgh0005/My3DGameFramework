...
namespace MGF3D
{
    ...
    {
        MGF_LOG_INFO("KTXCubeMapConverter: Starting CubeMap conversion process...");
        if (faceConfigs.size() != 6)
        {
            MGF_LOG_ERROR("KTXCubeMapConverter: Exactly 6 face configurations are required.");
            return false;
        }
    ...
    {
        MGF_LOG_INFO("KTXCubeMapConverter: [Step 1] Probing specs and creating KTX object...");
        ...
        if (!testData)
        {
            MGF_LOG_ERROR("KTXCubeMapConverter: Failed to load first face to probe specs: {}", faceConfigs[0].path);
            return false;
        }
        ...
        {
            MGF_LOG_ERROR("KTXCubeMapConverter: Failed to create KTX texture object (Error: {})", static_cast<int>(result));
            return false;
        }
    ...
    {
        MGF_LOG_INFO("KTXCubeMapConverter: [Step 2] Uploading 6 face images to storage...");
        ...
        for (uint32 i = 0; i < 6; ++i)
        {
            ...
            if (!data)
            {
                MGF_LOG_ERROR("KTXCubeMapConverter: Failed to load face {}: {}", i, faceConfigs[i].path);
                return false;
            }
            ...
            if (result != KTX_SUCCESS)
            {
                MGF_LOG_ERROR("KTXCubeMapConverter: Failed to set image for face {}. (Error: {})", i, (int)result);
                return false;
            }
        }
    ...
    void KTXCubeMapConverter::FlipImageHorizontal
    (
        unsigned char* data, 
        int32 width, int32 height, 
        int32 channels
    ...
    bool KTXCubeMapConverter::SaveToFile(ktxTexture2* texture, const String& outputPath)
    {
        MGF_LOG_INFO("KTXCubeMapConverter: [Step 3] Writing to file: {}", outputPath);
        ...
        if (result != KTX_SUCCESS)
        {
            MGF_LOG_INFO("KTXCubeMapConverter: Failed to write file: {} (Error: {})", outputPath, (int)result);
            return false;
        }

        MGF_LOG_INFO("KTXCubeMapConverter: Successfully generated cubemap -> {}", outputPath);
        return true;
    }
}