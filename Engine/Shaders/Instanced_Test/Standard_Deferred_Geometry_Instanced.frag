...
void main() 
{
    // 텍스처 기울기 저장 (Parallax 적용 후 밉맵 깨짐 방지)
    ...
    if (material.heightScale > 0.001) 
    {
        ...
        vec3 viewDirTangent = normalize(transpose(orthoTBN) * viewDir);
        texCoord = ParallaxMapping(TexCoords, viewDirTangent);
        if (texCoord.x > 1.0 || texCoord.y > 1.0 || texCoord.x < 0.0 || texCoord.y < 0.0) discard;
    }

    // 2. 샘플링 및 데이터 채우기
    float ao = material.hasORM ? textureGrad(material.orm, texCoord, ddx, ddy).r 
                               : textureGrad(material.ao, texCoord, ddx, ddy).r;
    gPosition = vec4(FragPos, ao);

    vec3 normalMap = texture(material.normal, texCoord).rgb * 2.0 - 1.0;
    gNormal = vec4(normalize(orthoTBN * normalize(normalMap)), material.shininess);

    vec4 albedoSample = texture(material.diffuse, texCoord);
    if (albedoSample.a * material.albedoFactor.a < 0.1) discard;
    gAlbedoSpec = vec4(albedoSample.rgb * material.albedoFactor.rgb, texture(material.specular, texCoord).r);
