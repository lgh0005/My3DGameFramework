...
namespace MGF3D
{
	...
	public:
		...
		SlabMemoryPool* GetSlabMemoryPool(usize size) const noexcept;
		LinearMemoryPool* GetLinearMemoryPool() const noexcept;
	...
	private:
		...
		SlabMemoryPool* m_slabMemoryPools[MGF3D::SlabBucketCount]; // 2. 관리 중인 버킷(Slab)들

		// Linear 기반 메모리 풀 영역 멤버들
		byte* m_linearMemoryPoolBuffer;
		LinearMemoryPool* m_linearMemoryPool;
	};
}
