...
{
	template<typename T>
	class SVector : public std::vector<T, SlabAllocator<T>>, public IContainer
	{
	public:
		using Base = std::vector<T, SlabAllocator<T>>;
		using Base::vector;
		SVector() = default;
		virtual ~SVector() = default;

	public:
		usize MemoryUsage() const override { return this->capacity() * sizeof(T); }
...
#include "Containers/Slab/SVector.inl"