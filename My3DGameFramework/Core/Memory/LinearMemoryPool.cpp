...
namespace MGF3D
{
	...
	void* LinearMemoryPool::Allocate(usize size, usize alignment) noexcept
	{
		...
		uintptr currentAddress = m_startAddress + m_offset.load();
		...
		// 남은 공간이 충분한지 확인
		if (m_offset.load() + totalNeeded > m_size) return nullptr;

		// 오프셋을 갱신하고 할당된 주소를 반환
		m_offset.fetch_add(totalNeeded);
