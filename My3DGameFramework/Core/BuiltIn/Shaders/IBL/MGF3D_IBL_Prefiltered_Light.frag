...
// 저도차 수열 (Low-discrepancy sequence) 생성
...
// GGX 미세면 모델에 기반하여, 거칠기에 따라 반사될 확률이 높은 방향(Half vector)을 추출
vec3 ImportanceSampleGGX(vec2 Xi, vec3 N, float roughness) 
{
    ...
    // 구면 좌표계 -> 직교 좌표계 (탄젠트 공간)
    vec3 H;
    H.x = cos(phi) * sinTheta;
    H.y = sin(phi) * sinTheta;
    H.z = cosTheta;
    
    // 탄젠트 공간 -> 월드 공간 변환 (up 벡터의 특이점(Singularity) 방지 처리)
