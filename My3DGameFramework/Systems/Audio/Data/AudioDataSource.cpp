...
namespace MGF3D
{
	AudioDataSource::AudioDataSource(ma_engine& engine) : m_audioEngine(engine) { }
	AudioDataSource::~AudioDataSource()
	{
		Unload();
	}

	bool AudioDataSource::Load(const String& filePath, uint32 flags)
	{
		Unload();
		...
		{
			MGF_LOG_ERROR("AudioDataSource: Failed to load {}", filePath.c_str());
			return false;
		}
